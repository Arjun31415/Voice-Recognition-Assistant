...
while int(time.time()) - t1 <= 7:
    ...
    MEMORY = 100
    avg_freq_buffer = (
        avg_freq_buffer*MEMORY +
        np.abs(freq_data)) / (MEMORY+1)
    obj_last = obj

    obj = np.real(np.dot(avg_freq_buffer[1:51], np.abs(
        freq_synth[1:100:2])) / np.dot(freq_synth[1:100:2], np.conj(freq_synth[1:100:2])))
    if obj > obj_last:
        fir_last = fir
    fir = fir_last.copy()
...
with wave.open(WAVE_OUTPUT_FILENAME, "wb") as f:
